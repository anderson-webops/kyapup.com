...
Optional machine imports use a separate credential. Leave `PHOTO_IMPORT_TOKEN_SHA256` unset unless an importer is needed. The unprivileged helper `node back-end/scripts/create-import-token.mjs` prints JSON with `token` and `sha256`: keep `token` only in the importing machine's protected configuration, add `PHOTO_IMPORT_TOKEN_SHA256=<sha256>` to `/etc/kyapup/api.env`, and restart the API. The credential can check import status and upload into the archive; it cannot administer the gallery or read private images. Remove the hash and restart to revoke it. See [the import API guide](../docs/import-api.md). No Apple Photos library access is part of server setup.
