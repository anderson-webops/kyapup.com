...
export VITESSE_RELEASE
VITESSE_RELEASE="v$(node -p 'require("./package.json").version')"
export VITESSE_COMMIT_SHA="$commit"
export VITESSE_DEPLOYED_AT
VITESSE_DEPLOYED_AT=$(date -u +%Y-%m-%dT%H:%M:%SZ)
node scripts/write-release-metadata.mjs
cp package.json package-lock.json .vitesse-release-prepared.json "$stage/"
...
receipt["checks"] = ["production-only locked install", "full and production backend audits", "registry signatures", "manifest and required paths", "isolated unpacked runtime", "readiness failure and recovery", "GET and HEAD minimal probes", "repeated-signal drain", "restart", "authenticated photo upload and image conversion", "publication and archived media authorization", "photo and settings persistence", "scoped machine import and duplicate preservation", "persistent import identities", "secondary activation boundary", "atomic concurrent secondary failures", "secondary lockout expiry and restart", "primary recovery during secondary lockout", "consistent photo and auth backup restore", "post-copier verification", "missing-module rejection"]
